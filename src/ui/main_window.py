...
class MainWindow(QMainWindow):
    ...
    def init_styles(self):
        ...
        # Agregamos un nombre de objeto para poder estilizarlo
        self.style_menu.setObjectName("settingsMenu")
        ...
        for style_name in self.style_sheets.keys():
            ...
            # Agregamos un nombre de objeto para los items
            action.setObjectName("menuItem")
    ...
    def init_tabs(self):
        ...
        self.tabs_counter = 0
        self.data_tab.column_selector.confirm_button.clicked.connect(
            self.create_linear_model_tab)
    ...
    def create_linear_model_tab(self):
        ...
        if not self.data_tab.selected_input_columns or not self.data_tab.selected_output_column:
            show_error(
                "Debe seleccionar columnas de entrada y salida antes de crear un modelo.", self)
            return

        # Check for null values in selected columns
        selected_columns = self.data_tab.selected_input_columns + \
            [self.data_tab.selected_output_column]
        has_nulls = self.data_tab.data[selected_columns].isnull().any().any()

        if has_nulls:
            show_error(
                "Hay valores nulos en los datos seleccionados. Por favor, aplique algún método de preprocesado antes de crear el modelo.", self)
            return
        ...
        if model_data:
            try:
                ...
                tab_index = self.tab_widget.addTab(
                    new_tab, f"Modelo {self.tabs_counter}")
                ...
                show_error(f"Error al crear la pestaña del modelo: {\
                           str(e)}", self)
                return False
